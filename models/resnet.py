...
def WideResNet_28_2_cifar(out_dim=7):
...
class TVResNet(nn.Module):
    def __init__(self, name='resnet18', pretrained=True, freeze_initial=False):
        super(TVResNet, self).__init__()
        weights = 'IMAGENET1K_V1' if pretrained else None
        try:
            self.model = getattr(tv_models, name)(weights=weights)
        except TypeError:
            self.model = getattr(tv_models, name)(pretrained=pretrained)
        
        self.last = self.model.fc
        self.model.fc = nn.Identity()

        if freeze_initial:
            # Freeze conv1, bn1, and layer1
            for param_name, param in self.model.named_parameters():
                if param_name.startswith('conv1') or param_name.startswith('bn1') or param_name.startswith('layer1') or param_name.startswith('layer2'):
...
def TVResNet18_pretrained():
    return TVResNet('resnet18', pretrained=True, freeze_initial=False)

def TVResNet34_pretrained():
    return TVResNet('resnet34', pretrained=True, freeze_initial=False)

def TVResNet50_pretrained():
    return TVResNet('resnet50', pretrained=True, freeze_initial=False)

def TVResNet18_pretrained_freeze():
    return TVResNet('resnet18', pretrained=True, freeze_initial=True)

def TVResNet34_pretrained_freeze():
    return TVResNet('resnet34', pretrained=True, freeze_initial=True)

def TVResNet50_pretrained_freeze():
    return TVResNet('resnet50', pretrained=True, freeze_initial=True)
